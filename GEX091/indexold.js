...
function getTransactionsTypeList(log, items) {
  ...
  const redoItems = transactionList.filter(x => ![...commitedItems, ...uncommitedItems].includes(x));

  return transactionList.map(item => {
    const isRedo = redoItems.indexOf(item) >= 0;
    const isCommited = commitedItems.indexOf(item) >= 0;
...
function logChanges(items, secondPrintData) {
  ...
  secondPrintData.forEach(item => console.log(`Transação ${item.transaction} ${item.type !== 'REDO' ? 'não ' : ''}realizou Redo`))
}
...
async function query(sql) {
  const client = new Client({
    database: 'db2', host: 'localhost', user: 'postgres', password: '1357924'
  });
