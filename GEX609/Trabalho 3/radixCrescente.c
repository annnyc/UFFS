...
void radixSort (int vetor[], int tamanho)
{
    ...
    while (aux/exp>0)
    {
        ...
        for (i = tamanho - 1 ; i >=0 ; i--)
            vetor2[--bucket[vetor[i]/exp%10]]=vetor[i];
        for (i=0; i< tamanho ; i++){
            vetor[i]=vetor2[i];
        }
        exp*=10;
...
int main(){
    ...
    inicio = clock();
    int tamanhoVetor, i;
    printf("Bem-vind@!\n Este é um Radix Sort crescente\n");
    ...
    for (i = 0; i < tamanhoVetor; i++){
        printf("%d\n", vetorRadix[i]);}
    fim = clock();
    printf( "O tempo de execucao do Radix Sort para ordenar %d valores em ordem crescente foi de %lu segundos.\n",tamanhoVetor, (fim - inicio) / CLOCKS_PER_SEC );
}